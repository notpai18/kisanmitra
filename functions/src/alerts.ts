...
interface Alert {
  id?: string;
  type: 'listing_progress' | 'listing_fulfilled' | 'listing_closing' | 'new_listing' | 'disease_alert';
  title: string;
  message: string;
  district: string;
  listingId: string;
 ...
 */
async function createAlert(alert: Omit<Alert, 'id' | 'createdAt' | 'read'>) {
 ...
 */
async function getUsersInDistrict(district: string): Promise<string[]> {
 ...
 */
export const onGroupListingItemAdded = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      const listing = listingDoc.data() as GroupListing;
      ...
      if (progressPercent >= 100) {
        ...
        await sendNotification(
          listing.organizerId,
          'Listing Fulfilled! 🎉',
          `${listing.groupName} has reached its target!`,
          { listingId, type: 'listing_fulfilled' }
        );
        ...
        for (const itemDoc of itemsSnapshot.docs) {
          ...
          if (participantId !== listing.organizerId) {
            await sendNotification(
              ...
              `${listing.crop} listing is now full. Delivery soon.`,
              { listingId, type: 'listing_fulfilled' }
            );
          }
        }
 ...
 */
export const onGroupListingCreated = functions.firestore
  .document('groupListings/{listingId}')
  .onCreate(async (snap, context) => {
    const listing = snap.data() as GroupListing;
 ...
 */
export const markAlertRead = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
 ...
 */
export const onCropReportHighSeverity = functions.firestore
  .document('users/{userId}/cropReports/{reportId}')
  .onCreate(async (snap, context) => {
    const report = snap.data() as CropReport;
    ...
    try {
      const district = report.district || report.location?.district;
      const state = report.state || report.location?.state;
