 ...
 */
export const processDailyTasks = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      for (const doc of oldSnapshot.docs) {
        ...
        await db.collection('groupListingsArchive').doc(doc.id).set({
          ...listingData,
 ...
 * - Market price trends
 */
export const processWeeklyDigest = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      const listingsByDistrict: Record<string, number> = {};
 ...
 */
export const triggerManualAlert = functions.https.onCall(async (data, context) => {
